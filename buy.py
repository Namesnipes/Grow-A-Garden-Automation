    ...
    while(len(toBuy) > 0):
        ...
        for imagePath in toBuy:
            ...
            while coords is None:
                ...
                if coords is None:
                    print(f"Item not found: {imagePath}. Scrolling down...")  # Debugging output to see which item is not found
                    # debug save screenshot
                    autoit.wheel("down", 1)  # Scroll down by one click to look at the next item in the shop
                    sleep(0.4)  # Wait for the scroll animation to finish
                    tries += 1
                    if tries > 10:  # If we have tried to find the item more than 10 times, break out of the loop
            ...
            if coords is not None: # Found item in the shop
                print(f"Found item: {imagePath} at coordinates: {coords}")  # Debugging output to see where the item was found
